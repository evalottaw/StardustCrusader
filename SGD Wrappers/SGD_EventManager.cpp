...
namespace SGD
{
	namespace SGD_IMPLEMENTATION
	{
		...
		public:
			...
			virtual bool		RegisterForEvent	( IListener* listener, const char* eventID )		override;
			virtual bool		UnregisterFromEvent	( IListener* listener, const char* eventID )		override;
		...
		private:
			...
			typedef std::multimap< EventID, IListener* >	ListenerMap;
	...
	namespace SGD_IMPLEMENTATION
	{
		...
		bool EventManager::Initialize( void )
		{
			...
			SGD_ASSERT( m_eStatus != E_INITIALIZED, "EventManager::Initialize - wrapper has already been initialized" );
		...
		{
			// Sanity-check the wrapper's status
			SGD_ASSERT( m_eStatus == E_INITIALIZED, "EventManager::Update - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;
			...
			while( m_qEvents.empty() == false )
			{
				...
				if( range.first != range.second )
				{
					...
					else 
					{
						...
						for( LMapIter iter = range.first; iter != range.second; ++iter )
						{
							if( dynamic_cast< const void* >( iter->second ) == eventPair.second )
							{
								vec.push_back( iter->second );
								break;
							}
		...
		{
			// Sanity-check the wrapper's status
			SGD_ASSERT( m_eStatus == E_INITIALIZED, "EventManager::Terminate - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;
		...
		//*************************************************************//
		// REGISTER FOR EVENT
		bool EventManager::RegisterForEvent( IListener* listener, const char* eventID )
		{	
			// Sanity-check the wrapper's status
			SGD_ASSERT( m_eStatus == E_INITIALIZED, "EventManager::RegisterForEvent - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;

			// Sanity-check the parameters
			SGD_ASSERT( listener != nullptr, "EventManager::RegisterForEvent - listener cannot be null" );
			if( listener == nullptr )
				return false;

			SGD_ASSERT( eventID != nullptr, "EventManager::RegisterForEvent - event ID cannot be null" );
			if( eventID == nullptr )
				return false;
		...
		bool EventManager::UnregisterFromEvent( IListener* listener, const char* eventID )
		{	
			// Sanity-check the wrapper's status
			SGD_ASSERT( m_eStatus == E_INITIALIZED, "EventManager::UnregisterFromEvent - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;

			// Sanity-check the listener parameter
			SGD_ASSERT( listener != nullptr, "EventManager::UnregisterFromEvent - listener cannot be null" );
			if( listener == nullptr )
				return false;
		...
		{
			// Sanity-check the wrapper's status
			SGD_ASSERT( m_eStatus == E_INITIALIZED, "EventManager::QueueEvent - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;

			// Sanity-check the parameter
			SGD_ASSERT( pEvent != nullptr, "EventManager::QueueEvent - event parameter cannot be null" );
			if( pEvent == nullptr )
				return false;

			
			// Queue the event
			m_qEvents.push( EventDestinationPair{ pEvent, destination } );
		...
		{
			// Sanity-check the wrapper's status
			SGD_ASSERT( m_eStatus == E_INITIALIZED, "EventManager::SendMessageNow - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;
			
			// Sanity-check the parameter
			SGD_ASSERT( pEvent != nullptr, "EventManager::SendEventNow - event parameter cannot be null" );
			if( pEvent == nullptr )
				return false;
			...
			if( range.first != range.second )
			{
				...
				else 
				{
					...
					for( LMapIter iter = range.first; iter != range.second; ++iter )
					{
						if( dynamic_cast< const void* >( iter->second ) == destination )
						{
							vec.push_back( iter->second );
							break;
						}
		...
		{
			// Sanity-check the wrapper's status
			SGD_ASSERT( m_eStatus == E_INITIALIZED, "EventManager::ClearEvents - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;
