...
// Uses _snprintf_s for debug text
#include <cstdio>
...
namespace SGD
{
	...
	namespace SGD_IMPLEMENTATION
	{
		...
		bool MessageManager::Initialize( void (*pfMessageProc)( const Message* ) )
		{
			...
			SGD_ASSERT( m_eStatus != E_INITIALIZED, "MessageManager::Initialize - wrapper has already been initialized" );
			if( m_eStatus == E_INITIALIZED )
				return false;

			// Sanity-check the parameter
			SGD_ASSERT( pfMessageProc != nullptr, "MessageManager::Initialize - callback function cannot be null" );
		...
		{
			// Sanity-check the wrapper's status
			SGD_ASSERT( m_eStatus == E_INITIALIZED, "MessageManager::Update - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;
		...
		{
			// Sanity-check the wrapper's status
			SGD_ASSERT( m_eStatus == E_INITIALIZED, "MessageManager::Terminate - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;
		...
		{
			// Sanity-check the wrapper's status
			SGD_ASSERT( m_eStatus == E_INITIALIZED, "MessageManager::QueueMessage - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;

			// Sanity-check the parameter
			SGD_ASSERT( pMsg != nullptr, "MessageManager::QueueMessage - message parameter cannot be null" );
			if( pMsg == nullptr )
				return false;
		...
		{
			// Sanity-check the wrapper's status
			SGD_ASSERT( m_eStatus == E_INITIALIZED, "MessageManager::SendMessageNow - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;
			
			// Sanity-check the parameter
			SGD_ASSERT( pMsg != nullptr, "MessageManager::SendMessageNow - message parameter cannot be null" );
			if( pMsg == nullptr )
				return false;
		...
		{
			// Sanity-check the wrapper's status
			SGD_ASSERT( m_eStatus == E_INITIALIZED, "MessageManager::ClearMessages - wrapper has not been initialized" );
			if( m_eStatus != E_INITIALIZED )
				return false;
